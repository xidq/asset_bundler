...
pub async fn encrypt_folder(
    ...
    poziom_kompresji: u8, 
    debug_create_lua_file: bool,) -> Result<(), io::Error > {
    ...
    let mut index = tokio::fs::File::create(index_file).await
            .map_err(|e| {
    ...
    let mut info_file: Option<tokio::fs::File> = None;
    if debug_create_lua_file {
        info_file = Some(tokio::fs::File::create(format!("{}{}", output_file, ".lua")).await
    ...
    output.write_all(&(file_entries.len() as u32).to_le_bytes()).await?;
    output.write_all(signature_bytes).await?;
    ...
    let mut current_offset = header_size;
        ...
        for i in 0..file_infos.len() {
            ...
            if toggle_encryption == 1 {
                chacha20::encrypt_chacha(&mut processed_data, password);
            }
        ...
        output.write_all(&processed_data).await?
    ...
    if  toggle_encryption==1{
        ...
        index.write_all(&idx_bytes).await?;
    }else{    index.write_all(&idx_bytes).await?;}
    ...
    if let Some(ref mut info_file) = info_file {
        ...
        // Nagłówki
        content.push_str("-- Plik informacyjny dla data.dat \n\n\n\n\n\n\n\n\n\n");
        ...
        // Zapisujemy hasło, jeśli nie jest zaszyfrowane
        if toggle_encryption == 0 {
            content.push_str(&format!("password = \"{}\"\n", password));
        } else {
            content.push_str("Nie zabezpieczone hasłem\n");
        }
    
        content.push_str("files = {\n");
    
        // Zapisujemy szczegóły plików
        for (type_name, type_name_id, type_name_variant, type_name_variant_size, unique_id, new_file_path, file_size, offset) in &file_infos {
        ...
        // Zapisujemy do pliku
        info_file.write_all(content.as_bytes()).await?;
    }
