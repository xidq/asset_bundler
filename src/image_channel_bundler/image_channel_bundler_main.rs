    ...
    let nowy_watek = thread::spawn(move||-> Result<(usize,usize), std::io::Error>{
        ...
        if all_successful {
            ...
            let valid_images: Vec<DynamicImage> = procesowane_rgb.into_iter()
                ...
                .collect();
            println!("każdy osobny plik przetworzony ok, wsio w vec co ma byc, teraz do zapisu");
            ...
            println!("Obrazy połączone i zapisane.");

        } else {
            println!("Nie wszystkie operacje zakończyły się sukcesem.");
        }
...
mod test_laczenia{
    ...
    fn testowanko_laczenia_fot(){
        ...
        let vec_u8: Arc<Mutex<Vec<u8>>> =Arc::new(Mutex::new(vec![4,1,8,4]));
